...
class PrivacyPolicyPage extends Page implements HasForms
{
    ...
    protected static ?string $title = 'سياسة الخصوصية';
    ...
    {
        $record = LegalPage::firstOrCreate(['key' => 'privacy'], ['title' => 'سياسة الخصوصية']);
    ...
    public function save(): void
    {
        ...
        $record = LegalPage::where('key', 'privacy')->first() ?? new LegalPage(['key' => 'privacy']);
        $record->fill($state);
        $record->save();

        Notification::make()
            ->title('تم حفظ سياسة الخصوصية بنجاح')
            ->success()
            ->send();
    }
}
