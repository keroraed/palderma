...
<section id="spotlight" class="reveal" style="max-width:1240px;margin:24px auto;padding:0 26px">
  <div style="text-align:center;max-width:680px;margin:0 auto 40px">
    <h2 style="font-size:clamp(28px,3.6vw,44px);font-weight:900;line-height:1.2;margin:0;color:#2a1620">{{ $sectionInfo->title ?? 'نبذة عن طبيب عيادة التجميل' }}</h2>
  </div>
  <div style="background:linear-gradient(135deg,#4d1022 0%,#6c1830 100%);border-radius:36px;overflow:hidden;color:#fff;display:grid;grid-template-columns:1fr 1.1fr;box-shadow:0 30px 60px -20px rgba(108,24,48,.5)" data-col="spot">
    <div data-spot-img style="position:relative;min-height:440px">
      <img src="{{ asset($spotlight->image) }}" alt="{{ $spotlight->name }}{{ $spotlight->specialty ? ' — ' . $spotlight->specialty : '' }}" loading="lazy" decoding="async" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center top;display:block">
    ...
    <div style="padding:60px 48px;display:flex;flex-direction:column;justify-content:center">
      ...
      <div data-spot-stats style="display:flex;gap:28px;margin-bottom:32px;padding-bottom:28px;border-bottom:1px solid rgba(255,255,255,.15)">
      ...
      <div style="margin-bottom:32px">
        ...
        <ul data-spot-quals style="margin:0;padding:0;list-style:none">
      ...
      <div>
        <a data-spot-cta href="{{ $spotlight->cta_href ?? '#book' }}" class="btn-hover-light-pink" style="display:inline-block;background:#fff;color:#6c1830;padding:14px 32px;border-radius:100px;font-weight:700;font-size:16px">{{ $spotlight->cta_label ?? 'احجز استشارة مع الدكتور' }}</a>
      </div>
    </div>
  </div>
</section>
