@props(['service'])
<div id="service-{{ $service->id }}" data-service-card data-service-id="{{ $service->id }}" class="card-hover-lift" style="background:#fff;border-radius:24px;padding:32px 24px;border:1px solid rgba(108,24,48,.1);box-shadow:0 10px 25px -10px rgba(108,24,48,.06);display:flex;flex-direction:column;justify-content:space-between;cursor:pointer;scroll-margin-top:110px">
  <div>
